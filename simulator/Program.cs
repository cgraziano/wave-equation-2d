// Runs two wave experiments and exports per-frame amplitude data as a
// JavaScript file that the static web viewer can load directly.
//
...
static List<string> RunScenario(WaveSolver solver, int totalSteps, int frameInterval)
{
...
static double[,] UniformMap(int height, int width, double speed = 1.0)
{
...
// A broad impulse passes through two narrow openings and produces an
// interference pattern on the far side.
// ---------------------------------------------------------------------------
Console.WriteLine("Running scenario 1: double-slit diffraction...");
{
    ...
    var frames = RunScenario(solver, TotalSteps, FrameInterval);
    WriteScenario("doubleSlit", "Double-Slit Diffraction",
...
// Two point sources fire simultaneously a fixed distance apart.
// Circular wavefronts overlap and create a standing interference pattern.
// ---------------------------------------------------------------------------
Console.WriteLine("Running scenario 2: two-source interference...");
{
    ...
    int centreX = W / 2;
    int centreY = H / 2;
    int separation = 24;   // distance between the two sources

    solver.AddImpulse(cx: centreX, cy: centreY - separation / 2, magnitude: 1.0, sigma: 2.0);
    solver.AddImpulse(cx: centreX, cy: centreY + separation / 2, magnitude: 1.0, sigma: 2.0);

    var frames = RunScenario(solver, TotalSteps, FrameInterval);
    WriteScenario("twoSource", "Two-Source Interference",
                   ...
                   int width, int height, List<string> frames)
{
    if (_sb.Length == 0)
    {
        _sb.AppendLine("// Auto-generated by Simulator — do not edit by hand.");
        _sb.AppendLine("const SCENARIOS = {");
    }

    _sb.AppendLine($"  {key}: {{");
    _sb.AppendLine($"    title: {JsonString(title)},");
    _sb.AppendLine($"    description: {JsonString(description)},");
    _sb.AppendLine($"    width: {width},");
    _sb.AppendLine($"    height: {height},");
    _sb.AppendLine($"    frameCount: {frames.Count},");
    _sb.AppendLine("    frames: [");
    for (int i = 0; i < frames.Count; i++)
    {
        string comma = i < frames.Count - 1 ? "," : "";
        _sb.AppendLine($"      \"{frames[i]}\"{comma}");
    }
    _sb.AppendLine("    ]");
    _sb.AppendLine("  },");
}
